// Firebase
  ...
  function openPopover(menu, anchor){
    ...
    // Προσπάθησε να ευθυγραμμίσεις αριστερά με το κουμπί,
    // σίγουρα όμως μέσα στο viewport.
    let left = ar.left;
    // Αν χωράει καλύτερα να "κουμπώσει" δεξιά από το κουμπί:
  ...
  function renderGifResults(items){
    ...
    items.forEach(g=>{
      ...
      const img=document.createElement('img'); img.src=still || url; img.loading='lazy';
      div.appendChild(img);
  ...
  replyClose.onclick=()=>{ replyState=null; replyPreview.style.display='none'; };
